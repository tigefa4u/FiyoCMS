...
function parseFilesystem($displayResultForEditing = TRUE) {
	...
	$a = breakSession();	
	$b = $SETTINGS[PSNG_SCAN_WEBSITE];
	$c = $SETTINGS[PSNG_TIMEOUT];
	$d = PSNG_TIMEOUT_NONE;
	$e = $SETTINGS[PSNG_TIMEOUT_ACTION]; 
	$f = $SETTINGS[PSNG_TIMEOUT_ACTION]; 
	$g = PSNG_TIMEOUT_ACTION_WEBSITE;
	
	//echo " $a && $b != null ($c == $d ||$e == null $f ==)";
