...
$(document).ready(function(){
	...
	$("#type").change(function(){	
		...
		if(type == 'featured' || type == 'category' || type == 'archives'){	
			...
			if(type == 'category') {
				$("#link").val("?app=article&view="+type+"&id="+cate+"&type="+format);
				$(".category").removeClass("invisible");	
			}
			else
				$("#link").val("?app=article&view="+type+"&type="+format);
	...
	if(type == 'item'){		
  	...
  	$(".pop_up2").click(function() {
		$.ajax({
			url: "../apps/app_article/libs/article_menu.php",
			data: "access",
		...
		<div class="acmain open">
			...
			<tr class="item invisible" >
				...
				<td>
					...
					<input type="text" value="<?php echo oneQuery('article','id',$aId,'title'); ?>" id="pg" size="20" readonly /> 
			...
			<tr class="per_page">
				<td class="djudul tooltip" title="%a = Author, %c = Category<br>%d = Day, %f = Day(1-31)<br>%m = Month, %n = Month(1-12)<br>%y = Year<br><hr>default : '<b>by %a on %f %m %y in %c</b>'">Panel Format</td>
				<td>
